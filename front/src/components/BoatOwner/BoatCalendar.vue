...
export default {
  ...
  mounted() {
    BoatReservationService.getAllReservationsOfOwnerForCalendar(LogInService.userId).then(res => {
  ...
  methods: {
    handleEventClick(arg){
      ...
      if(new Date() < arg.event.end){
        alert("THIS BOAT RESERVATION IS NOT ABLE TO BE REPORTED, BECAUSE IT IS NOT STARTED");
        return;
      }
    ...
    makeReport(adventureId){
      this.report = {comment: '', reservationType: 'BR', badReport: false, customerAppear: true,
